...
namespace SystemMonitoringLogger.Services
{
    public class MqttService
    {
        ...
        async void OnReceiveMessage(object sender, MqttMsgPublishEventArgs e)
        {
            var message = Encoding.Default.GetString(e.Message);
            var sensorValue = JsonConvert.DeserializeObject<SystemInfo>(message);
            var context = _provider.ServiceProvider.GetService<SystemMonitoringLoggerContext>();
            await context.SystemInfo.AddAsync(sensorValue);
